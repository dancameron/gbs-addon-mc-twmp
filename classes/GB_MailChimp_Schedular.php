...
class GB_MailChimp_Schedular extends Group_Buying_Controller {
	...
	public function gbs_top_purchase_hook($purchase, $is_voucher = FALSE) {
		...
		$api = GB_MailChimp_Utility::validate_mailchimp_api_key(self::$api_key);
		if (!$api) {
		...
		$groups = GB_MailChimp_Utility::get_mailchimp_groups( $api, $list_id );
		...
		$order_id = GB_MailChimp_Utility::get_deal_order_record( $purchase_id );
		self::gbs_write_to_file("Order ID: " . $order_id);

		$order_status = GB_MailChimp_Utility::get_deal_order_status( $order_id );
		...
		// Fetch User
		$user = GB_MailChimp_Utility::get_user($user_id);
		...
		if (isset($user)) {
			...
			if (!$api->errorCode) {
				if (GB_MailChimp_Utility::check_non_renewed_user( $user['user_email']))
					GB_MailChimp_Utility::update_non_renewed_user( $user['user_email'], date('Y-m-d H:i:s') );
				else
					GB_MailChimp_Utility::add_non_renewed_user( $user['user_email'], date('Y-m-d H:i:s') );
			} 
	...
	public static function gbs_top_purchase_scheduled_emails() {
		...
		$last_cron_time = GB_MailChimp_Utility::get_last_cron_time( 'top_purchase_scheduled_emails' );
		...
		if (isset($scheduled_templates)) {
			foreach ($scheduled_templates as $key=>$value) {
				...
				// Fetch Purchase IDs for Top Deals after Last Cron
				$purchase_ids = GB_MailChimp_Utility::get_purchases_for_top_deal_on_date( $deal_ids, $product_purchase_date );
				self::gbs_write_to_file("Purchases: " . count($purchase_ids));

				if (count($purchase_ids)) {
					// Fetch Users
					foreach ($purchase_ids as $k=>$v) {
						$user_id = GB_MailChimp_Utility::get_deal_purchase_record( $v->post_id );
					...
					foreach ($users as $k=>$v) {
						// Fetch User
						$user = GB_MailChimp_Utility::get_user($v);
						// Check User
						if (isset($user)) {
...
//							$yesterday_date = date('dmY', mktime(0, 0, 0, date('m'), date('d')-1, date('y')));
		...
		GB_MailChimp_Utility::update_last_cron_time( 'top_purchase_scheduled_emails' , $current_time );
		return true;
	}
	...
	public static function gbs_non_renewed_trigger() {
		...
		// Fetch Last Cron Time
		$last_cron_time = GB_MailChimp_Utility::get_last_cron_time( 'non_renewed_trigger' );
		self::gbs_write_to_file("Last Cron Time: " . $last_cron_time);
		...
		$users = GB_MailChimp_Utility::get_non_renewed_users_before_purchase_date( $last_purchase_time );
				...
				if ($member['success']) {
					...
					if (isset($member['data'][0]['merges']['GROUPINGS'])) {
						...
						if ($add_to_group) {
							...
							if (!$api->errorCode) {
								GB_MailChimp_Utility::update_non_renewed_user( $value->email, date('Y-m-d H:i:s') );
							}
						}
						else
		...
		// Update Cron Time
		GB_MailChimp_Utility::update_last_cron_time( 'non_renewed_trigger' , $current_time );
